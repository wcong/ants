...
def inside_project():
    ...
    if scrapy_module is not None:
        try:
            ...
            warnings.warn("Cannot import ants settings module %s: %s" % (scrapy_module, exc))
        else:
            return True
...
def project_data_dir(project='default'):
    ...
    if cfg.has_option(DATADIR_CFG_SECTION, project):
        ...
        if not scrapy_cfg:
            raise NotConfigured("Unable to find ants.cfg file to infer project data dir")
        d = abspath(join(dirname(scrapy_cfg), '.ants'))
    if not exists(d):
        os.makedirs(d)
    return d
