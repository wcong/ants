...
import ants
from ants.utils.python import unicode_to_str
from ants.settings import overridden_settings
...
def _adapt_eventdict(eventDict, log_level=INFO, encoding='utf-8', prepend_level=True):
    ...
    # ignore non-error messages from outside ants
    if ev.get('system') != 'ants' and not ev['isError']:
        return
...
def scrapy_info(settings):
    ...
    if sflo:
        msg("Scrapy %s started (bot: %s)" % (ants.__version__, \
            settings['BOT_NAME']))

        msg("Optional features available: %s" % ", ".join(ants.optional_features),
            level=INFO)
