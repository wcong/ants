...
from ants.http import Request, FormRequest, XmlRpcRequest, Headers, HtmlResponse
...
class RequestTest(unittest.TestCase):
    ...
    def test_url(self):
        """Request url tests"""
        r = self.request_class(url="http://www.ants.org/path")
        self.assertEqual(r.url, "http://www.ants.org/path")

        # url quoting on creation
        r = self.request_class(url="http://www.ants.org/blank%20space")
        self.assertEqual(r.url, "http://www.ants.org/blank%20space")
        r = self.request_class(url="http://www.ants.org/blank space")
        self.assertEqual(r.url, "http://www.ants.org/blank%20space")

        # url encoding
        r1 = self.request_class(url=u"http://www.ants.org/price/\xa3", encoding="utf-8")
        r2 = self.request_class(url=u"http://www.ants.org/price/\xa3", encoding="latin1")
        self.assertEqual(r1.url, "http://www.ants.org/price/%C2%A3")
        self.assertEqual(r2.url, "http://www.ants.org/price/%A3")
