...
from ants.utils.test import get_testlog, docrawl
from ants.tests.spiders import SimpleSpider, SingleRequestSpider
from ants.tests.mockserver import MockServer


class HTTPSProxy(controller.Master, Thread):

    def __init__(self, port):
        password_manager = http_auth.PassManSingleUser('ants', 'ants')
...
class ProxyConnectTestCase(TestCase):
    ...
    def setUp(self):
        ...
        time.sleep(1.0)
        os.environ['http_proxy'] = 'http://ants:ants@localhost:8888'
        os.environ['https_proxy'] = 'http://ants:ants@localhost:8888'
    ...
    def test_https_noconnect(self):
        os.environ['https_proxy'] = 'http://ants:ants@localhost:8888?noconnect'
        spider = SimpleSpider("https://localhost:8999/status?n=200")
        yield docrawl(spider)
        self._assert_got_response_code(200)
        os.environ['https_proxy'] = 'http://ants:ants@localhost:8888'
