...
from ants.utils.misc import load_object, arg_to_iter, walk_modules

__doctests__ = ['ants.utils.misc']

class UtilsMiscTestCase(unittest.TestCase):

    def test_load_object(self):
        obj = load_object('ants.utils.misc.load_object')
        assert obj is load_object
        self.assertRaises(ImportError, load_object, 'nomodule999.mod.function')
        self.assertRaises(NameError, load_object, 'ants.utils.misc.load_object999')

    def test_walk_modules(self):
        mods = walk_modules('ants.tests.test_utils_misc.test_walk_modules')
        expected = [
            'ants.tests.test_utils_misc.test_walk_modules',
            'ants.tests.test_utils_misc.test_walk_modules.mod',
            'ants.tests.test_utils_misc.test_walk_modules.mod.mod0',
            'ants.tests.test_utils_misc.test_walk_modules.mod1',
        ]
        self.assertEquals(set([m.__name__ for m in mods]), set(expected))

        mods = walk_modules('ants.tests.test_utils_misc.test_walk_modules.mod')
        expected = [
            'ants.tests.test_utils_misc.test_walk_modules.mod',
            'ants.tests.test_utils_misc.test_walk_modules.mod.mod0',
        ]
        ...
        mods = walk_modules('ants.tests.test_utils_misc.test_walk_modules.mod1')
        expected = [
            'ants.tests.test_utils_misc.test_walk_modules.mod1',
        ]
        self.assertEquals(set([m.__name__ for m in mods]), set(expected))
